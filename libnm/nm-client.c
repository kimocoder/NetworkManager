...
#define _main_context_push_main_if_necessary(self) \
	({ \
		NMClient *_self = (self); \
		\
		_assert_main_context_is_current_source (_self, dbus_context); \
		\
		nm_g_main_context_push_thread_default_if_necessary (NM_CLIENT_GET_PRIVATE (_self)->main_context); \
	})
                              ...
                              gpointer user_data)
{
	...
	dbus_context = _main_context_push_main_if_necessary (self);
